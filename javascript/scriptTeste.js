...
//funçao para passar o mês e se o 
function nextMonth() {
  month++;
  if (month > 11) {
    month = 0;
...
function getActiveDay(date) {
  ...
  dataToSend.append("dia", dayName);
  dataToSend.append("data_completa", `${date} ${months[month]} ${year}`);
  ...
  .then(data => {
    ...
    // você pode mostrar alguma notificação se quiser
  })
  .catch(error => {
    console.error("Erro ao enviar para o PHP:", error);
  });
}
...
addEventSubmit.addEventListener("click", () => {
  ...
  //checa se o formato de horario esta correto
  ...
  if (
    ...
    alert("Formato de horário errado");
    return;
  }
  ...
  eventsArr.forEach((event) => {
    if (
      ...
      //alterar para que title e materia sejam iguais
      event.events.forEach((event) => {
        if (event.title === eventTitle || event.type === event ) {
          eventExist = true;
        }
      });
    }
